...
const initPayment = async (
    ...
    try {
        const paymentBaseUrl = baseUrl + basePath + '/payments';
        const data = {
            store_id,
            store_passwd,
            is_live,
...
const validatePayment = async (payload: SSLValidationPayload) => {
    try {
        const response = await axios.get(
            `https://sandbox.sslcommerz.com/validator/api/validationserverAPI.php?val_id=${payload.val_id}&store_id=${store_id}&store_passwd=${store_passwd}&format=json`,
        );
        return response.data;
    } catch (err) {
