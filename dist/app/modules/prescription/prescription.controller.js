"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PrescriptionControllers = void 0;
const http_status_1 = __importDefault(require("http-status"));
const catchAsync_1 = __importDefault(require("../../utils/catchAsync"));
const sendResponse_1 = __importDefault(require("../../utils/sendResponse"));
const prescription_service_1 = require("./prescription.service");
const constants_1 = require("../../constants");
const pick_1 = __importDefault(require("../../utils/pick"));
const prescription_constant_1 = require("./prescription.constant");
const createPrescription = (0, catchAsync_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { user } = req;
    const result = yield prescription_service_1.PrescriptionServices.createPrescription(req.body, user);
    (0, sendResponse_1.default)(res, {
        statusCode: http_status_1.default.CREATED,
        message: 'Prescription created successfully',
        data: result,
    });
}));
const getPatientPrescriptions = (0, catchAsync_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { user } = req;
    const filters = (0, pick_1.default)(req.query, prescription_constant_1.prescriptionPatientFilters);
    const query = (0, pick_1.default)(req.query, constants_1.queryFilters);
    const result = yield prescription_service_1.PrescriptionServices.getPatientPrescriptions(filters, query, user);
    (0, sendResponse_1.default)(res, {
        statusCode: http_status_1.default.OK,
        message: 'Prescription fetched successfully',
        meta: result.meta,
        data: result.data,
    });
}));
const getAllPrescriptions = (0, catchAsync_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const filters = (0, pick_1.default)(req.query, prescription_constant_1.prescriptionFilters);
    const query = (0, pick_1.default)(req.query, constants_1.queryFilters);
    const result = yield prescription_service_1.PrescriptionServices.getAllPrescriptions(filters, query);
    (0, sendResponse_1.default)(res, {
        statusCode: http_status_1.default.OK,
        message: 'Prescription fetched successfully',
        meta: result.meta,
        data: result.data,
    });
}));
exports.PrescriptionControllers = {
    createPrescription,
    getPatientPrescriptions,
    getAllPrescriptions,
};
